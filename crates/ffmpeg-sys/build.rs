...
fn build() -> io::Result<()> {
	...
	println!("cargo-rerun-if-env-changed=FFMPEG_SYS_NEXT_BUILD_EXTRA_ARGS");

	if let Ok(extra_args) = env::var("FFMPEG_SYS_NEXT_BUILD_EXTRA_ARGS") {
	...
	// run ./configure
	let output = configure.output().unwrap_or_else(|_| panic!("{:?} failed", configure));
...
fn check_features(include_paths: Vec<PathBuf>, infos: &[(&'static str, Option<&'static str>, &'static str)]) {
	...
	for &(_, feature, var) in infos {
		...
		// Also find out if defined or not (useful for cases where only the definition
		// of a macro can be used as distinction)
	...
	for &(ffmpeg_version_flag, lavc_version_major, lavc_version_minor) in ffmpeg_lavc_versions.iter() {
		let search_str = format!(
...
fn main() {
		...
		// Check additional required libraries.
		{
			...
			extra_linker_args
				...
				.for_each(|lib_search_path| {
					println!("cargo:rustc-link-search=native={}", lib_search_path.to_str().unwrap());
				})
		}
	...
	// Use prebuilt library
		...
		if ffmpeg_dir.join("lib/amd64").exists() && env::var("CARGO_CFG_TARGET_ARCH").as_deref() == Ok("x86_64") {
			println!(
				"cargo:rustc-link-search=native={}",
				ffmpeg_dir.join("lib/amd64").to_string_lossy()
			);
		} else if ffmpeg_dir.join("lib/armhf").exists() && env::var("CARGO_CFG_TARGET_ARCH").as_deref() == Ok("arm") {
			println!(
				"cargo:rustc-link-search=native={}",
				ffmpeg_dir.join("lib/armhf").to_string_lossy()
			);
		} else if ffmpeg_dir.join("lib/arm64").exists() && env::var("CARGO_CFG_TARGET_ARCH").as_deref() == Ok("aarch64") {
			println!(
				"cargo:rustc-link-search=native={}",
	...
	// Fallback to pkg-config
		...
		pkg_config::Config::new().statik(statik).probe("libavutil").unwrap();
	...
	if let Some(hwcontext_drm_header) = maybe_search_include(&include_paths, "libavutil/hwcontext_drm.h") {
