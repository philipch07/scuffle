...
impl<S: AsyncReadWrite> Session<S> {
	...
	async fn do_handshake(&mut self, handshaker: &mut HandshakeServer) -> Result<bool, SessionError> {
		...
		while bytes_len < handshake::RTMP_HANDSHAKE_SIZE {
			let buf = tokio::time::timeout(Duration::from_millis(2500), self.io.read()).await.map_err(|_| SessionError::BytesIO(BytesIOError::ClientClosed))??;
			bytes_len += buf.len();
	...
	async fn do_ready(&mut self) -> Result<bool, SessionError> {
		...
		if self.skip_read {
			...
			let data = tokio::time::timeout(Duration::from_millis(2500), self.io.read()).await.map_err(|_| SessionError::BytesIO(BytesIOError::ClientClosed))??;
	...
	async fn write_data(&mut self, data: Bytes) -> Result<(), SessionError> {
		if !data.is_empty() {
			tokio::time::timeout(Duration::from_secs(2), self.io.write(data)).await.map_err(|_| SessionError::BytesIO(BytesIOError::ClientClosed))??;
		}
